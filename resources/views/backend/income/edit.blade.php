    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <span class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                <a href="{{ route('admin.income.index') }}" class="text-white"><i class="fas fa-university fa-sm text-white-50"></i> {{ __('Income') }}</a> / 
                ...
                <form method="POST" action="<?=route('admin.income.update', $income)?>" enctype="multipart/form-data">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $income->title) }}"/>
                                    @error('title')
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="date" class="form-control datepicker @error('date') is-invalid @enderror" value="{{ old('date', $income->date->format('d-m-Y') ) }}"/>
                                    @error('date')
                                        <div class="invalid-feedback">
                                            <strong>{{ $message }}</strong>
                                        </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="amount" class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $income->amount) }}"/>
                                    @error('amount')
                                        <div class="invalid-feedback">
                                            <strong>{{ $message }}</strong>
                                        </div>
                                ...
                                <div class="form-group col-md-12">
                                    <label>{{ __('Note') }}</label>
                                    <textarea name="note" id="note" cols="30" rows="3" class="form-control @error('note') is-invalid @enderror">{{ old('note', $income->note) }}</textarea>
                                    @error('note')
                                        <div class="invalid-feedback">
                                            <strong>{{ $message }}</strong>
                                        </div>
                        ...
                        <div class="card-footer">
                            <button type="submit" class="btn btn-primary">{{ __('Update income') }}</button>
                        </div>
                    </div>
                </form>
            </div>
