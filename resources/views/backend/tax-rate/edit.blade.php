    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-xl-6 col-md-6">
                <form method="POST" action="<?=route('admin.tax-rate.update', $taxRate)?>">
                    @csrf
                    @method('PUT')
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <input type="number" min="0" name="percent" class="form-control @error('percent') is-invalid @enderror" value="{{ old('percent', $taxRate->percent) }}" step=".01"/>
                                    @error('percent')
                                        <div class="invalid-feedback">
                                            <strong>{{ $message }}</strong>
                                        </div>
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <select name="status" class="form-control @error('status') is-invalid @enderror">
                                        @foreach(trans('tax_statuses') as $taxStatusKey => $taxStatus)
                                            <option value="{{ $taxStatusKey }}" {{ (old('status', $taxRate->status) == $taxStatusKey) ? 'selected' : '' }}>{{ $taxStatus }}</option>
                                        @endforeach
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <textarea name="description" id="description" cols="30" rows="3" class="form-control @error('description') is-invalid @enderror">{{ old('description', $taxRate->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">
                                            <strong>{{ $message }}</strong>
                                        </div>
                        ...
                        <div class="card-footer">
                            <button type="submit" class="btn btn-primary">{{ __('Update tax rate') }}</button>
                        </div>
                    </div>
                </form>
            </div>
